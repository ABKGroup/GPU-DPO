...
bool boundaryCheck(const float* x,
    ...
    for (int i = 0; i < num_movable_nodes; ++i) {
        ...
        if (node_xl + precision < xl || node_xh > xh + precision || node_yl + precision < yl ||
            ...
            printf("[INFO GPU-DPO] ERROR: node %d (%g, %g, %g, %g) out of boundary\n", i, node_xl, node_yl, node_xh, node_yh);
            legal_flag = false;
        }
    }
    return legal_flag;
...
bool siteAlignmentCheck(const float* x,
    ...
    for (int i = 0; i < num_movable_nodes; ++i) {
        ...
        if (std::abs(row_id_f - row_id) > precision) {
            printf("[INFO GPU-DPO] ERROR: node %d (%g, %g) failed to align to row %d (%g, %g), gap %g, yl %g, row_height %g",
                         i,
                         node_xl,
                         node_yl,
                         row_id,
        ...
        if (std::abs(site_id_f - site_id) > precision) {
            printf("[INFO GPU-DPO] ERROR: node %d (%g, %g) failed to align to row %d (%g, %g) and site; xl %g, site_width %g",
                         i,
                         node_xl,
                         node_yl,
                         row_id,
    ...
    for (int i = 0; i < num_movable_nodes; ++i) {
        ...
        if (region_id < num_regions) {
            ...
            if (node_area > 0) {  // not consumed by boxes within a region
                ...
                printf("[INFO GPU-DPO] ERROR: node %d (%g, %g, %g, %g), out of fence region %d: %s",
                             i,
                             node_xl,
                             node_yl,
                             node_xh,
            ...
            if (j > 0) {
                ...
                if (node_id < num_movable_nodes || prev_node_id < num_movable_nodes)  // ignore two fixed nodes
                {
                    ...
                    if (prev_site_xh > cur_site_xl) {
                        printf(
                            "[INFO GPU-DPO] ERROR: row %d (%g, %g), overlap node %d (%g, %g, %g, %g) with "
...
bool legalityCheckKernelCPU(const float* x,
    ...
    if (!boundaryCheck(x, y, node_size_x, node_size_y, scale_factor, xl, yl, xh, yh, num_movable_nodes)) {
        legal_flag = false;
        std::cerr << "[INFO GPU-DPO] boundary check error!" << std::endl;
    }

    // check row and site alignment
    if (!siteAlignmentCheck(x, y, site_width, row_height, scale_factor, xl, yl, num_movable_nodes)) {
        legal_flag = false;
        std::cerr << "[INFO GPU-DPO] site alignment check error!" << std::endl;
    }
    ...
    if (!overlapCheck(x,
                      ...
                      num_movable_nodes)) {
        legal_flag = false;
        std::cerr << "[INFO GPU-DPO] overlap check error!" << std::endl;
    }
    ...
    if (!fenceRegionCheck(x,
                          ...
                          scale_factor)) {
        legal_flag = false;
        std::cerr << "[INFO GPU-DPO] fence region check error!" << std::endl;
    }

    if (!legal_flag) {
        printf("[INFO GPU-DPO] placement legality check error!");
    }
