...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void DetailedReorderer::run(DetailedMgr* mgrPtr, DetailedPlaceData& db, const std::string& command)
{
...
void compute_reorder_instances(const DetailedPlaceData& db,
...
// void compute_row_conflict_graph(const DetailedPlaceData& db, KReorderState<float>& state) {
...
// void compute_independent_rows(const DetailedPlaceData& db, KReorderState<float>& state) {
//   compute_independent_rows(db, state.row_graph, state.independent_rows);
// }

/// @brief distribute cells to rows
void make_row2node_map(const DetailedPlaceData& db,
                       const float* vx,
                       const float* vy,
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, 1)
  for (int i = 0; i < db.num_sites_y; ++i) {
    ...
    if (!row2nodes.empty()) {
      ...
      std::sort(row2nodes.begin(), row2nodes.end(), [&](int node_id1, int node_id2) {
        float x1 = vx[node_id1] + db.node_size_x[node_id1] / 2;
        float x2 = vx[node_id2] + db.node_size_x[node_id2] / 2;
        return x1 < x2 || (x1 == x2 && node_id1 < node_id2);
      });
    }
  }
...
inline __device__ void compute_position(const DetailedPlaceData& db,
                                        const KReorderState<float>& state,
...
__global__ void compute_instance_net_boxes(DetailedPlaceData db, KReorderState<float> state, int group_id, int offset) {
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < group_size; i += blockDim.x * gridDim.x) {
    ...
    if (K > 0) {
      float segment_xl = db.x[row2nodes[0]];
      float segment_xh = db.x[row2nodes[K - 1]];
      float row_yl = db.yl + inst.row_id * db.row_height;
...
__global__ void compute_reorder_hpwl(DetailedPlaceData db, KReorderState<float> state, int group_id, int offset) {
  __shared__ int group_size;
  ...
  float target_x[MAX_K];
  float target_sizes[MAX_K];
    ...
    if (valid_flag) {
      ...
      if (db.num_regions) {
        for (int idx = 0; idx < K; ++idx) {
          ...
          float node_xl = target_x[permuted_offset];
          float node_yl = db.y[node_id];
          if (!db.inside_fence(node_id, node_xl, node_yl)) {
            cost = cuda::numeric_limits<float>::max();
            break;
          }
        }
      }
      if (cost == 0) {
        ...
        for (int idx = 0; idx < instance_nets_size; ++idx) {
          auto& instance_net = instance_nets[idx];
          float bxl = instance_net.bxl;
          float bxh = instance_net.bxh;

          for (int j = 0; j < K; ++j) {
            int flag = (1 << j);
            if ((instance_net.node_marker & flag)) {
              int permuted_offset = permutation[j];
              float other_node_xl = target_x[permuted_offset];
...
__global__ void apply_reorder(DetailedPlaceData db, KReorderState<float> state, int group_id, int offset) {
  __shared__ int group_size;
  if (threadIdx.x == 0) {
    group_size = state.reorder_instances.size(group_id);
  }
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < group_size; i += blockDim.x * gridDim.x) {
    ...
    if (K > 0) {
      ...
      for (int i = 0; i < K; ++i) {
        int node_id = row2nodes[i];
        float xx = target_x[permutation[i]];
        if (db.x[node_id] != xx) {
...
__global__ void compute_node2inst_map(DetailedPlaceData db, KReorderState<float> state, int group_id, int offset) {
  __shared__ int group_size;
  if (threadIdx.x == 0) {
    group_size = state.reorder_instances.size(group_id);
  }
...
__global__ void compute_net_markers(DetailedPlaceData db, KReorderState<float> state) {
...
__global__ void print_net_markers(DetailedPlaceData db, KReorderState<float> state) {
...
__global__ void compute_instance_nets(DetailedPlaceData db, KReorderState<float> state, int group_id, int offset) {
  __shared__ int group_size;
  if (threadIdx.x == 0) {
    group_size = state.reorder_instances.size(group_id);
  }
...
__global__ void unique_instance_nets(DetailedPlaceData db, KReorderState<float> state, int group_id) {
  __shared__ int group_size;
  if (threadIdx.x == 0) {
    group_size = state.reorder_instances.size(group_id);
  }
...
__global__ void print_costs(KReorderState<float> state, int group_id, int offset) {
  if (blockIdx.x == 0 && threadIdx.x == 0) {
...
__global__ void print_best_permute_id(KReorderState<float> state, int group_id, int offset) {
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    printf("[INFO GPU-DPO] group_id %d, offset %d, %s\n", group_id, offset, __func__);
    for (int i = 0; i < state.reorder_instances.size(group_id); ++i) {
...
__global__ void print_instance_nets(KReorderState<float> state, int group_id, int offset) {
...
__global__ void print_instance_net_bboxes(KReorderState<float> state, int group_id, int offset) {
  assert(blockDim.x == 1 && gridDim.x == 1);
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    printf("[INFO GPU-DPO] group_id %d, offset %d, %s\n", group_id, offset, __func__);
    int size = state.reorder_instances.size(group_id);
...
__global__ void check_instance_nets(DetailedPlaceData db, KReorderState<float> state, int group_id) {
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    for (int i = 0; i < state.reorder_instances.size(group_id); ++i) {
...
__global__ void print_pos(DetailedPlaceData db, int group_id, int offset) {
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    printf("[INFO GPU-DPO] group_id %d, offset %d, pos[%d]\n", group_id, offset, db.num_movable_nodes);
    for (int i = 0; i < db.num_movable_nodes; ++i) {
      printf("[INFO GPU-DPO] [%d] = %g, %g\n", i, db.x[i], db.y[i]);
    }
  }
}

__global__ void reset_state(DetailedPlaceData db, KReorderState<float> state, int group_id) {
    __shared__ int group_size;
    __shared__ int group_size_with_permutation;
  if (threadIdx.x == 0) {
    group_size = state.reorder_instances.size(group_id);
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < group_size_with_permutation; i += blockDim.x * gridDim.x) {
    state.costs[i] = cuda::numeric_limits<float>::max();
  }
  ...
  for (int group_id = 0; group_id < state.reorder_instances.size1; ++group_id) {
    ...
    if (group_size) {
      for (int offset = 0; offset < state.K; offset += state.K / 2) {
        ...
        reduce_min_2d_cub<float, 32><<<group_size, 32>>>(state.costs, state.best_permute_id, group_size, state.num_permutations);
                            ...
                            const std::vector<std::string>& args)
{
  ...
  std::vector<float> host_node_space_x(db.num_movable_nodes);
  ...
  printf("[INFO GPU-DPO] %lu permutations\n", host_permutations.size());
  ...
  // initialize cpu db from db
  {
    ...
    allocateCopyCpu(cpu_db.net_mask, db.net_mask, db.num_nets, int);
    ...
    allocateCopyCpu(cpu_db.x, db.x, db.num_nodes, float);
    ...
    for (int i = 0; i < cpu_db.num_sites_y; ++i) {
      for (unsigned int j = 0; j < host_row2node_map_left.at(i).size(); ++j) {
        ...
        if (node_id < db.num_movable_nodes) {
          ...
          float space_xl = cpu_db.x[node_id];
          float space_xh = cpu_db.xh;
          ...
          float node_size_x = cpu_db.node_size_x[node_id];
          if (!(space >= node_size_x)) {
            printf("[INFO GPU-DPO] Assertion failed: space >= node_size_x — space %g, node_size_x[%d] %g, original space (%g, %g), site_width %g\n", space, node_id, node_size_x, space_xl, space_xh, db.site_width);
          }
        }
      }
    }
  ...
  // initialize cuda state
  {
    ...
    allocateCuda(state.costs, state.reorder_instances.size2 * state.num_permutations, float);
    allocateCuda(state.best_permute_id, state.reorder_instances.size2, int);
    allocateCuda(state.instance_nets, state.reorder_instances.size2 * MAX_NUM_NETS_PER_INSTANCE, InstanceNet<float>);
  ...
  for (int p = 1; p <= passes; p++) {
    ...
    printf("[INFO GPU-DPO] Pass %3d of k-reordering; hpwl %.3f => %.3f (imp. %g%%)\n", 
           p, 
           hpwls[0],
           hpwls[p],
