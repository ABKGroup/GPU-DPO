...
void prepare_kmeans(const GpuData& db,
                    const IndependentSetMatchingState& state,
                    KMeansState<int>& kmeans_state) {
    ...
    // set weights to 1.0
    fill_array(kmeans_state.weights, kmeans_state.num_seeds, (int)1.0);
}
...
__global__ void kmeans_find_centers_kernel(GpuData db,
  ...
  for (int center_id = threadIdx.x; center_id < kmeans_state.num_seeds; center_id += ThreadsPerBlock) {
      ...
      int center_x =
          kmeans_state.centers_x[center_id] / KMeansState<int>::scale;
      int center_y =
          kmeans_state.centers_y[center_id] / KMeansState<int>::scale;
      int weight = kmeans_state.weights[center_id];

      int distance = kmeans_distance(node_x, node_y, center_x, center_y) * weight;
...
__global__ void init_kmeans_seeds_kernel(GpuData db,
                                         IndependentSetMatchingState state,
                                         KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < kmeans_state.num_seeds) {
    ...
    kmeans_state.centers_x[i] = db.x[node_id] * KMeansState<int>::scale;
    kmeans_state.centers_y[i] = db.y[node_id] * KMeansState<int>::scale;
  }
}

void init_kmeans_seeds(const GpuData& db,
                       IndependentSetMatchingState& state,
                       KMeansState<int>& kmeans_state) {
  init_kmeans_seeds_kernel<<<ceilDiv(kmeans_state.num_seeds, 256), 256>>>(db, state, kmeans_state);
}

__global__ void reset_kmeans_partition_sizes_kernel(GpuData db,
                                                    IndependentSetMatchingState state,
                                                    KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < kmeans_state.num_seeds) {
...
__global__ void compute_kmeans_partition_sizes_kernel(GpuData db,
                                                      IndependentSetMatchingState state,
                                                      KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void reset_kmeans_centers_kernel(GpuData db,
                                            IndependentSetMatchingState state,
                                            KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < kmeans_state.num_seeds) {
...
__global__ void compute_kmeans_centers_sum_kernel(GpuData db,
                                                  IndependentSetMatchingState state,
                                                  KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < state.num_selected) {
    ...
    assert(node_id < db.num_movable_nodes);
    // scale up for fixed point numbers
    atomicAddWrapper<typename KMeansState<int>::coordinate_type>(
        kmeans_state.centers_x + center_id, db.x[node_id] * KMeansState<int>::scale);
    atomicAddWrapper<typename KMeansState<int>::coordinate_type>(
        kmeans_state.centers_y + center_id, db.y[node_id] * KMeansState<int>::scale);
  }
}

__global__ void compute_kmeans_centers_div_kernel(GpuData db,
                                                  IndependentSetMatchingState state,
                                                  KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < kmeans_state.num_seeds) {
...
void kmeans_update_centers(const GpuData& db,
                           IndependentSetMatchingState& state,
                           KMeansState<int>& kmeans_state) {
...
__global__ void compute_kmeans_weights_kernel(GpuData db,
                                              IndependentSetMatchingState state,
                                              KMeansState<int> kmeans_state) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < kmeans_state.num_seeds) {
    ...
    if (s > state.set_size) {
      auto ratio = s / (int)state.set_size;
...
void kmeans_update_weights(const GpuData& db,
                           IndependentSetMatchingState& state,
                           KMeansState<int>& kmeans_state) {
  compute_kmeans_weights_kernel<<<ceilDiv(kmeans_state.num_seeds, 256), 256>>>(db, state, kmeans_state);
}

void kmeans_collect_sets_cuda2cpu(const GpuData& db,
                                  IndependentSetMatchingState& state,
                                  KMeansState<int>& kmeans_state) {
...
void collect_independent_sets(const GpuData& db,
                              IndependentSetMatchingState& state,
                              KMeansState<int>& kmeans_state,
                              CpuData& host_db,
