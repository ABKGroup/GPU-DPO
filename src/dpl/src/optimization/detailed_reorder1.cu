...
//   // work with left edge of the cells
//   for (int i = 0; i < size; i++) {
//     const Node* node = nodes[istrt + i];
//     int lp, rp;
//     arch->getCellPadding(node, lp, rp);
//     total_padding += (lp + rp);
//     total_width += node->getWidth().v;
//   }

//   int total_required = total_width + total_padding;
//   if ((right_limit - left_limit) < total_required) {
//     return false;
//   }
//   // distribute the whitespace evenly among cells
//   const int space_per_cell = ((right_limit - left_limit) 
//                               - (total_width + total_padding)) / size;
//   const int site_width = arch->getRow(0)->getSiteWidth();
//   const int site_per_cell_total = space_per_cell / site_width;
//   const int site_per_cell_right = site_per_cell_total >> 1;
//   const int site_per_cell_left = site_per_cell_total - site_per_cell_right;
//   for (int i = 0; i < size; i++) {
//     if (total_width + total_padding + site_per_cell_right * site_width 
//         < right_limit - left_limit) {
//       total_padding += site_per_cell_right * site_width;
//       right[i] += site_per_cell_right * site_width;
//     }
//     if (total_width + total_padding + site_per_cell_left * site_width 
//         < right_limit - left_limit) {
//       total_padding += site_per_cell_left * site_width;
//       left[i] += site_per_cell_left * site_width;
//     }
//   }
//   total_required = total_width + total_padding;
//   if ((right_limit - left_limit) < total_required) {
//     return false;
//   }
//   return true;
...
// ///////////////////////////////////////////////////////////////////////////////
// ///////////////////////////////////////////////////////////////////////////////
// std::vector<std::vector<ReorderInstance>> 
//   compute_instances(GpuData& db,
//                     Architecture* arch,
//                     DetailedMgr* mgrPtr,
//                     Network* network,
//                     int windowSize,
...
//   std::vector<std::vector<ReorderInstance>> reorder_instances(segment_groups.size());

//   for (int group_id = 0; group_id < segment_groups.size(); group_id++) {
//     for (int seg_id : segment_groups[group_id]) {
//       DetailedSeg* segPtr = mgrPtr->getSegment(seg_id);
//       const int segId = segPtr->getSegId();
//       const int rowId = segPtr->getRowId();

//       const std::vector<Node*>& nodes = mgrPtr->getCellsInSeg(segId);
//       if (nodes.size() < 2) {
//         continue;
//       }
//       mgrPtr->sortCellsInSeg(segId);

//       int j = 0;
//       const int n = (int) nodes.size();
//       // Extract all single-height (for now) movable cells in the segment
//       while (j < n) {
//         while (j < n && arch->isMultiHeightCell(nodes[j])) {
//           ++j;
//         }
//         const int jstrt = j;
//         while (j < n && arch->isSingleHeightCell(nodes[j])) {
//           ++j;
//         }
//         const int jstop = j - 1;

//         // Single height cells in [jstrt,jstop].
//         // Partition these cells into windows in the segments
//         // Only window is evaluated in each of these due to dependency issues
//         for (int i = jstrt; i + windowSize <= jstop; i += windowSize) {
//           int istrt = i;
...
//           int rightLimit = segPtr->getMaxX().v;
//           if (nextPtr != nullptr) {
//             int leftPadding, rightPadding;
//             arch->getCellPadding(nextPtr, leftPadding, rightPadding);
//             rightLimit = std::min((nextPtr->getLeft().v - leftPadding), rightLimit);
//           }
//           const Node* prevPtr = (istrt != 0) ? nodes[istrt - 1] : nullptr;
//           int leftLimit = segPtr->getMinX().v;
//           if (prevPtr != nullptr) {
//             int leftPadding, rightPadding;
//             arch->getCellPadding(prevPtr, leftPadding, rightPadding);
//             leftLimit = std::max(prevPtr->getRight().v + rightPadding, leftLimit);
//           }
...
//             }
//             reorder_instances[group_id].push_back(inst);
//           }
//         }
//       }
//     }
//   }
//   printf("[INFO GPU-DPO] Evaluating %d reorder instances\n", reorder_instances.size());
//   return reorder_instances;
// }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
// void reorder(GpuData& db,
//              KReorderState& state,
//              const std::vector<std::vector<KReorderInstance>>& host_reorder_instances) {
//   const int K = state.K;
//   const int num_permutations = state.num_permutations;
//   state.reorder_instances.initialize(host_reorder_instances);
//   allocateCuda(state.costs, state.reorder_instances.size2 * num_permutations, float);
...
//     for (int offset = 0; offset < K; offset += K / 2) {
//       checkCuda(cudaMemset(state.costs, 0, sizeof(float) * state.reorder_instances.size2 * num_permutations));
//       checkCuda(cudaMemset(state.best_permute_id, 0, sizeof(int) * state.reorder_instances.size2));
//       checkCuda(cudaMemset(state.device_num_moved, 0, sizeof(int)));

//       compute_costs<<<ceilDiv(group_size * num_permutations, 256), 256>>>(
//           db, state, group_id, offset);

//       reduce_min_2d_cub<int, 32><<<group_size, 32>>>(
//           state.costs, state.best_permute_id, group_size, num_permutations);
                            ...
                            const std::vector<std::string>& args)
{
  ...
  // std::vector<int> orig_node2segs(db_.num_nodes);
  // for (int i = 0; i < mgrPtr_->getNumSegments(); i++) {
  //   const std::vector<Node*>& nodes = mgrPtr_->getCellsInSeg(i);
  //   for (Node* node : nodes) {
  ...
  //   mgrPtr_->removeCellFromSegment(node, orig_node2segs[i]);
  //   mgrPtr_->addCellToSegment(node, node2segs[i]);
  // }

  // // Extract independent row groups once on host
  // auto segment_groups = group_independent_segments(mgrPtr_, db_.num_threads);
  // // Compute all reorder instances for all the independent row groups
  // auto reorder_instances = compute_instances(db_, arch_, mgrPtr_, network_, windowSize_, state, segment_groups);

  // // Generate all the permutations needed
  // std::vector<std::vector<int>> permutations = quick_perm(windowSize_);
  // std::vector<int> permutations_flat(permutations.size() * windowSize_);
  // for (int i = 0; i < permutations.size(); i++) {
  //   std::copy(host_permutations[i].begin(), host_permutations[i].end(),
  //           host_permutations_flat.begin() + i * windowSize_);
  // }
  // state.num_permutations = host_permutations.size();
  // allocateCopyCUDA(state.permutations, host_permutations_flat.data(),
  //                state.num_permutations * windowSize_);
  
  // state.reorder_instances.initialize(reorder_instances);
