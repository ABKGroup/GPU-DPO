...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
DetailedGlobalSwap::DetailedGlobalSwap(GpuData* gpuData)
                             ...
                             std::vector<std::string>& args)
{
  ...
  for (int p = 1; p <= passes; p++) {
    ...
    globalSwap(db_, state);
    checkCuda(cudaDeviceSynchronize());

    curr_hpwl = compute_total_hpwl(db_, db_.x, db_.y, state.net_hpwls);

    printf("[INFO GPU-DPO] Pass {:3d} of global swaps; hpwl is {:.6e}.", p, (double) curr_hpwl);
  ...
  printf("[INFO GPU-DPO] End of global swaps; objective is {:.6e}, "
