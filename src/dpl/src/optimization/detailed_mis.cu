...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
DetailedMis::DetailedMis(GpuData* gpuData)
    : db_(gpuData)
{
}
...
void DetailedMis::run(DetailedMgr* mgrPtr, std::vector<std::string>& args)
{
  ...
  state.num_moved = 0;
  state.large_number = ((db_.xh - db_.xl) + (db_.yh - db_.yl)) * set_size;
  ...
  iota<<<ceilDiv(db.num_movable_nodes, 512), 512>>>(state.ordered_nodes, db_.num_movable_nodes);
  ...
  allocateCuda(state.solution_costs,
                ...
                int);
  allocateCuda(state.net_hpwls, db_.num_nets, doubintle);
  ...
  for (int p = 1; p <= passes; p++) {
    ...
    printf("[INFO GPU-DPO] Pass {:3d} of matching; objective is {:.6e}.", p, curr_hpwl);
  ...
  //   curr_obj = curr_disp;
  // }
  printf("[INFO GPU-DPO] End of matching; objective is {:.6e}, improvement is {:.2f} percent.", curr_hpwl,
      hpwl_imp);
