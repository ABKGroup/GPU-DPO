...
void DetailedGlobalSwap

void DetailedGlobalSwap::globalSwap(DetailedPlaceDB& db) {
  ...
  for (int i = 0; i < db.num_movable_nodes; i += state.batch_size) {
    ...
    reset_candidate_costs<<<ceilDiv(db.num_movable_nodes, 512), 512>>>(db, state);
    compute_candidate_costs<<<ceilDiv(db.num_movable_nodes, 512), 512>>>(db, state);

    // apply parallel reduction to find the best candidate
    reduce_min_2d_cub<float, 256><<<idx_end - idx_bgn, 256>>>(state.candidates, state.max_num_candidates);
    // use single thread to apply candidate solution
    apply_candidates<<<1, 1>>>(db, state, idx_bgn, idx_end);
  }
}
